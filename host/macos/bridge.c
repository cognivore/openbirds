...
// std/num/int64, host, pixel, framebuffer, viewport, lzw, gif,
// runtime, scroll, font, text, truetype/*, window/article*, scene,
// app, ttf, hello).
extern void kk_hello__main__init(kk_context_t* _ctx);
extern void kk_hello__main__done(kk_context_t* _ctx);

// Generated by Koka from `scene.kk`, `runtime.kk`, `app.kk`, and
// `ttf.kk`. Each ref is process-lifetime; the bridge dups for
// each call so Koka can consume the argument per its calling
// conventions.
...
extern kk_unit_t kk_app_handle_tap_scrolled(kk_ref_t s, kk_ref_t sr, kk_ref_t pc,
...
extern kk_integer_t kk_app_would_close_tap(kk_ref_t sr, kk_ref_t pc, kk_ref_t spec,
    kk_integer_t vx, kk_integer_t vy, kk_context_t* _ctx);
extern kk_std_core_types__tuple4 kk_app_cached_x_rect(kk_ref_t pc, kk_context_t* _ctx);
extern kk_std_core_types__tuple4 kk_app_cached_close_rect(kk_ref_t pc, kk_context_t* _ctx);
extern kk_string_t kk_app_tap_debug_line(kk_ref_t sr, kk_ref_t pc, kk_ref_t spec,
    kk_integer_t vx, kk_integer_t vy, kk_context_t* _ctx);
...
extern kk_unit_t kk_app_handle_pan_start(kk_ref_t sr, kk_ref_t spec,
...
extern kk_ref_t kk_app_new_page_cache(kk_context_t* _ctx);
extern kk_ref_t kk_app_new_spec_cell(kk_context_t* _ctx);
extern kk_unit_t kk_app_set_spec(kk_ref_t c,
...
static kk_ref_t borrow_pcache(kk_context_t* ctx) {
    if (!g_pcache_init) {
        g_pcache      = kk_app_new_page_cache(ctx);
        g_pcache_init = true;
    }
...
static kk_ref_t borrow_spec(kk_context_t* ctx) {
    if (!g_spec_init) {
        g_spec      = kk_app_new_spec_cell(ctx);
        g_spec_init = true;
    }
...
// --- pixel framebuffer ------------------------------------------------------
//
// Calls into Koka's `app::frame-rgba`, which returns a Koka
...
void openbirds_render_frame(double now_seconds,
    ...
    kk_app_set_spec(kk_ref_dup(spec, ctx),
...
void openbirds_tap(int32_t x, int32_t y,
    ...
    kk_integer_t hk = kk_app_would_close_tap(
    ...
    {
        kk_string_t ds = kk_app_tap_debug_line(
            borrow_scroll(ctx), borrow_pcache(ctx), borrow_spec(ctx),
            kk_integer_from_int32(x, ctx),
            kk_integer_from_int32(y, ctx), ctx);
...
void openbirds_pan_start(double x, double y, double t) {
    pthread_mutex_lock(&g_lock);
    kk_context_t* ctx = ensure_ctx();
    // Routed through app.handle-pan-start so the Koka side can
    // hydrate the cached viewport-spec and run the scrollbar
    // hit-test before deciding pan vs. scrubber.
    kk_app_handle_pan_start(borrow_scroll(ctx), borrow_spec(ctx),
                               x, y, t, ctx);
    pthread_mutex_unlock(&g_lock);
}
